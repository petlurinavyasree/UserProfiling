#!/usr/bin/env python3
# course: TCSS555
# User profile in social media - imageCNN1
# date: 10/10/2017
# name: Team 4 - Iris Xia
# description: Python file to generate gender prediction from image using simple CNN
import sys, getopt
import os
...
# ---------------------------------------------------------------------
# Method for gender prediction using simple CNN
#
...
def predictGender(inputfile, width, height):
    ...
    model = Sequential()
    model.add(Conv2D(64, (7, 7), strides=(4, 4), input_shape=input_shape, bias_initializer='zeros', padding="valid"))
    ...
    model.add(Conv2D(128, (3, 3), padding="same", bias_initializer='zeros'))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="valid"))
    ...
    model.load_weights("gender78.hdf5")
    # end of load model
